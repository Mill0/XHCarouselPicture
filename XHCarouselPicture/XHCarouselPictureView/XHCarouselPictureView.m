...
#define Margin 10
#define scHeight self.scrollView.frame.size.height
#define scWith self.scrollView.frame.size.width


#import "XHCarouselPictureView.h"

@interface XHCarouselPictureView ()<UIScrollViewDelegate>

@property (nonatomic, assign) ClickBlock clickBlock;  //点击图片block回调
...
@property (nonatomic, strong) UIImageView *otherImageView; //辅助滚动的imageView
...
@implementation XHCarouselPictureView
...
- (void)setImageArray:(NSArray *)imageArray{
    ...
    for (int i = 0; i < imageArray.count; i++) {
        if ([imageArray[i] isKindOfClass:[UIImage class]]) {
            ...
            [_images addObject:[UIImage imageNamed:@"placeholderImage.jpg"]];
...
//设置文字颜色
- (void)setDesLabelTextColor:(UIColor *)desLabelTextColor{
    _desLabelTextColor = desLabelTextColor;
    self.describeLabel.textColor = desLabelTextColor;
}

#pragma mark 设置定时器时间
- (void)setSwitchTime:(NSTimeInterval)switchTime {
...
- (void)startTimer {
    ...
    //如果定时器已开启，先停止再重新开启
    if (self.mTimer) [self stopTimer];
    self.mTimer = [NSTimer timerWithTimeInterval:_switchTime < 1? DEFAULTTIME : _switchTime target:self selector:@selector(nextPage) userInfo:nil repeats:YES];
...
- (void)setScrollViewContentSize {
    if (_images.count > 1) {
        self.scrollView.contentSize = CGSizeMake(scWith * 5, 0);
        self.scrollView.contentOffset = CGPointMake(scWith * 2, 0);
        self.currImageView.frame = CGRectMake(scWith * 2, 0, scWith, scHeight);
        
        [self startTimer];
    } else {
        self.scrollView.contentSize = CGSizeZero;
        self.scrollView.contentOffset = CGPointZero;
        self.currImageView.frame = CGRectMake(0, 0, scWith, scHeight);
    }
}
...
- (void)scrollViewDidScroll:(UIScrollView *)scrollView {
    ...
    if (offsetX < scWith * 2) {//right
        self.otherImageView.frame = CGRectMake(scWith, 0, scWith, scHeight);
        self.nextIndex = self.currIndex - 1;
        if (self.nextIndex < 0) self.nextIndex = _images.count - 1;
        if (offsetX <= scWith) {
            [self changeToNext];
        }
    } else if (offsetX > scWith * 2){//left
        self.otherImageView.frame = CGRectMake(CGRectGetMaxX(_currImageView.frame), 0, scWith, scHeight);
